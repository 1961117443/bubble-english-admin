...
import { getPrintDevSelector,getAllTemplateData } from '@/api/system/printDev'
...
const mutations = {
  ...
  SET_PRINT_ALL_TREE: (state, printTree) => {
    state.printDevAllTree = printTree
  }
}
...
const actions = {
  getDictionaryAll({ commit }) {
    return new Promise(async(resolve, reject) => {
      await getDictionaryAll().then(res => {
  ...
  // 获取所有模块关联的打印模板
  getAllTemplateData({ commit }) {
    return new Promise((resolve, reject) => {
      getAllTemplateData().then(res => {
        commit('SET_PRINT_ALL_TREE', res.data)
        resolve(res.data)
      }).catch(error => {
        reject(error)
      })
  ...
  getPrintTemplate({ state, dispatch }, key) {
    return new Promise(async resolve => {
      let list = {},
        data = [] 
      if (!state.printDevAllTree || Object.keys(state.printDevAllTree).length === 0) {
        list = await dispatch('getAllTemplateData')
      } else {
        list = state.printDevAllTree
      }
      if (key && Object.hasOwnProperty.call(list, key)) {
        data = list[key] || [];
      }
      return resolve(data) 
    })
