...
const whiteList = ['/login', '/auth-redirect', '/jump','switchRole','/quote/order/share','/logistics/quote/order/share'] // no redirect whitelist

router.beforeEach(async (to, from, next) => {
  // start progress bar
  NProgress.start()
  
  store.dispatch('settings/setSlideClass', (to.meta && to.meta.slideClass) ? to.meta.slideClass :'')
  store.dispatch('settings/setTagsView', !!to.meta ? to.meta.tagsView :'')
  if (!store.state.settings.loadGlobal) {
    await store.dispatch('settings/globalConfig')
  }
  if (!store.getters.multiTenancy || store.getters.tenantLoggedIn) {
    await store.dispatch('settings/getSystemTitle')
  }
