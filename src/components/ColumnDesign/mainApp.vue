  ...
  <div class="columnDesign-container">
    <div class="main-board">
      ...
      <div class="sort-box">
        ...
        <el-table :data="columnData.sortList" class="QT-common-table" ref="dragTableSort"
          :row-key="row=>row.prop">
          <el-table-column align="center" label="拖动" width="50">
            <template>
              <i class="drag-handler icon-qt icon-qt-darg" style="cursor: move;font-size:20px"
    ...
    <div class="right-board">
      ...
      <div class="field-box">
        <div class="columnList" v-show="currentTab==='sort'">
          <el-table :data="sortOptions" class="QT-common-table" height="100%"
  ...
  created() {
    ...
    const loop = (data, parent) => {
      ...
      if (data.__config__ && data.__config__.qtKey) {
        ...
        if (data.__config__.layout === "colFormItem" && data.__vModel__ && data.__vModel__.indexOf('_qt_') < 0 && visibility) {
          list1.push(data)
        }
      }
    }
    loop(getDrawingList())
    this.list = list
    let options = list.filter(o => noColumnShowList.indexOf(o.__config__.qtKey) < 0)
    let searchOptions = list.filter(o => noSearchList.indexOf(o.__config__.qtKey) < 0)
    let sortOptions = list1.filter(o => noColumnShowList.indexOf(o.__config__.qtKey) < 0)
    this.columnOptions = options.map(o => ({
      label: o.__config__.label,
      prop: o.__vModel__,
      align: 'left',
      qtKey: o.__config__.qtKey,
      sortable: false,
      width: null
    }));
