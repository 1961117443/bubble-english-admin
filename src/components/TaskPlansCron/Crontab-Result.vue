<template>
  <div class="popup-result">
    <el-card>
      <p class="title">最近{{sumcc}}次运行时间</p>
      <!-- <el-button @click="expressionChange">计算结果</el-button> -->
        ...
        goMouth: for (let Mi = MIdx; Mi < MDate.length; Mi++) {
          ...
          goDay: for (let Di = DIdx; Di < DDate.length; Di++) {
            ...
            if (this.dayRule == 'lastDay') {
              ...
              const _index = this.dayRuleSup.indexOf(thisWeek)
              // console.log(_index)
              if (_index < 0) {
              ...
              // this.dayRuleSup[0] - 1 - thisWeek  

              // //获取每月1号属于第几周
              // const thisWeekNum = Math.ceil((thisDD - thisWeek + 1) / 7);
              // console.log(thisWeekNum)
              // console.log(thisWeek)
              // console.log(thisDD)
              // console.log(thisWeekNum * 7 + this.dayRuleSup[0] - thisWeek + 1)
              // console.log(thisWeekNum * 7 + this.dayRuleSup[0] - thisWeek + 1 - thisWeek)

              const target = this.dayRuleSup[0] - 1 + 7; //需要星期几

              if (thisWeek > target) {
                DD = target + 7 - thisWeek
              } else {
                if (thisWeek == (target % 7)) {

                } else {
                  DD += ((target - thisWeek) % 7)
                }
                // DD = (target % 7)  - thisWeek     
              }

              DD += (this.dayRuleSup[1] - 1) * 7
