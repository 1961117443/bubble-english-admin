...
<template>
  <el-dialog :title="title" :close-on-click-modal="false"
    class="QT-dialog QT-dialog_center transfer-dialog" lock-scroll append-to-body
    ...
    <QTTransfer :loading="loading" :treeData="treeData" v-model="selectedData" :type="type"
      ref="QTTransfer" v-else />
...
<script>
import QTTransfer from '@/components/QT-transfer'
import userTransfer from '@/components/QT-userTransfer'
import roleTransfer from './roleTransfer'

export default {
  name: 'org-transfer',
  components: { QTTransfer, userTransfer, roleTransfer },
  props: {
    value: {
      type: Array,
      default: () => []
    ...
    dataInit() {
      ...
      this.$nextTick(async () => {
        if (this.type === 'user') {
          ...
          this.$refs.QTTransfer && (this.$refs.QTTransfer.filterText = '')
          this.treeData = res
