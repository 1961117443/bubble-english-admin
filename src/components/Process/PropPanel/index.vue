...
<template>
  <el-drawer size="550px" class="drawer QT-common-drawer" :visible.sync="visible" @close="cancel"
      ...
      <template v-for="(item, index) in pconditions">
        <el-row class="condition-list" :key="index">
          ...
          <el-col :span="7" class="filedValue">
            ...
            <template v-else-if="['date','createTime', 'modifyTime'].includes(item.qtKey)">
            ...
            <template v-else-if="['comSelect','currOrganize'].includes(item.qtKey)">
              <comSelect v-model="item.filedValue" placeholder="请选择" clearable
                @change="onConditionListChange(arguments,item)" />
            </template>
            <template v-else-if="['depSelect'].includes(item.qtKey)">
              <depSelect v-model="item.filedValue" placeholder="请选择" clearable
                @change="onConditionObjChange(arguments,item)" />
            </template>
            <template v-else-if="['userSelect','createUser','modifyUser'].includes(item.qtKey)">
              <userSelect v-model="item.filedValue" placeholder="请选择" hasSys clearable
                @change="onConditionObjChange(arguments,item)" />
            </template>
            <template v-else-if="['posSelect','currPosition'].includes(item.qtKey)">
              <posSelect v-model="item.filedValue" placeholder="请选择" clearable
                @change="onConditionObjChange(arguments,item)" />
            </template>
            <template v-else-if="item.qtKey==='address'">
              <QTAddress v-model="item.filedValue" placeholder="请选择" :level="item.level" clearable
                @change="onConditionListChange(arguments,item)" />
            </template>
            <template v-else>
          ...
          <el-scrollbar class="config-scrollbar">
            <el-form label-position="top" :model="subFlowForm" class="pd-10">
              ...
              <el-form-item label="子流程表单">
                <QT-TreeSelect :options="flowOptions" v-model="subFlowForm.flowId"
      ...
      <el-tabs style="height:100%;">
            ...
            <el-form label-position="top" class="pd-10">
              <el-form-item label="操作设置">
                ...
                <div class="per-cell" v-if="startForm.hasPrintBtn">
                  ...
                  <QT-TreeSelect :options="printTplList" v-model="startForm.printId"
        ...
        <el-tab-pane label="表单权限">
          <div class="form-auth-table">
            <el-table :data="getFormOperates()" class="QT-common-table" size="mini" height="100%">
            ...
            <el-form label-position="top" class="pd-10">
              <el-form-item label="操作设置">
                ...
                <div class="per-cell" v-if="approverForm.hasPrintBtn">
                  <p style="width:112px"></p>
                  <QT-TreeSelect :options="printTplList" v-model="approverForm.printId"
                    placeholder="请选择打印模板" lastLevel clearable></QT-TreeSelect>
                </div>
              </el-form-item>
    ...
    <el-dialog title="数据传递" :close-on-click-modal="false" :visible.sync="ruleVisible"
      class="QT-dialog QT-dialog_center rule-dialog" lock-scroll append-to-body width='700px'>
...
const requiredDisabled = (qtKey) => {
  return ['billRule', 'createUser', 'createTime', 'modifyTime', 'modifyUser', 'currPosition', 'currOrganize', 'table'].includes(qtKey)
}
const getDataType = (data) => {
  if (!data.__config__ || !data.__config__.qtKey) return ''
  const qtKey = data.__config__.qtKey
  if (['numInput', 'date', 'rate', 'slider'].includes(qtKey)) {
    return 'number'
  } else if (['checkbox', 'uploadFz', 'uploadImg', 'cascader', 'comSelect', 'address'].includes(qtKey)) {
    return 'array'
  } else if (['select', 'depSelect', 'posSelect', 'userSelect', 'treeSelect'].includes(qtKey)) {
...
},
{
  ...
  __vModel__: 'qtFlowOperatorUserId',
},
{
  __config__: {
    label: '当前操作用户名',
  ...
  __vModel__: 'qtFlowOperatorUserName',
}]
export default {
  ...
  computed: {
    ...
    usedFormItems() {
      ...
      const loop = (data, parent) => {
        ...
        if (data.__vModel__ && data.__config__.qtKey !== 'table') list.push(data)
      }
      loop(getDrawingList())
  ...
  methods: {
    ...
    initFormOperates(target) {
      ...
      if (!formOperates.length) {
        const loop = (data, parent) => {
          if (!data) return
          if (data.__vModel__) {
            const isTableChild = parent && parent.__config__ && parent.__config__.qtKey === 'table'
            res.push({
              ...
              requiredDisabled: requiredDisabled(data.__config__.qtKey) || data.__config__.required,
              qtKey: data.__config__.qtKey,
    ...
    */
    ...
    fieldNameChange(val, item, i) {
      ...
      item.qtKey = obj.__config__.qtKey
      item = { ...item, ...obj }
    ...
    onConditionDateChange(val, item) {
      ...
      item.filedLabel = this.qt.toDate(val, format)
    },
