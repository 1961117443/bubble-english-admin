...
const layouts = {
  colFormItem(h, scheme) {
    ...
    const Item = config.qtKey === 'cascader'
  ...
  rowFormItem(h, scheme) {
    ...
    if (scheme.__config__.qtKey === 'tab') {
      return (
    ...
    if (scheme.__config__.qtKey === 'collapse') {
      return (
    ...
    if (scheme.__config__.qtKey === 'table') {
    ...
    if (scheme.__config__.qtKey === 'card') {
      return (
    ...
    if (scheme.__config__.qtKey === 'row') {
      if (scheme.type === 'flex') {
...
function buildListeners(scheme) {
  ...
  if (scheme.on) {
    ...
    Object.keys(scheme.on).forEach(key => {
      ...
      const func = this.qt.getScriptFunc.call(this, str);
      if (!func) return
      listeners[key] = params => {
        if (key === 'change') {
          let data = ''
          if (['select', 'radio', 'checkbox'].includes(config.qtKey)) {
            const options = scheme.__slot__.options
            if (scheme.multiple || config.qtKey === 'checkbox') {
              let _data = []
  ...
  methods: {
    initFormData(componentList, formData) {
      ...
      componentList.forEach(cur => {
        ...
        if (cur.__config__.qtKey == 'table') return
    ...
    buildRules(componentList, rules) {
      componentList.forEach(cur => {
        ...
        if (config.children && config.qtKey !== 'table') this.buildRules(config.children, rules)
      })
    },
    onLoad(formConfCopy) {
      if (!formConfCopy || !formConfCopy.funcs || !formConfCopy.funcs.onLoad) return
      const onLoadFunc = this.qt.getScriptFunc.call(this, formConfCopy.funcs.onLoad)
    ...
    comSet(field, prop, value) {
      ...
      const loop = list => {
        for (let i = 0; i < list.length; i++) {
          ...
          if (item.__vModel__ && item.__vModel__ === prop) {
            switch (field) {
              case 'disabled':
                ...
                break;
              case 'options':
                if (dyOptionsList.indexOf(item.__config__.qtKey) > -1) {
          ...
          if (item.__config__ && item.__config__.qtKey !== 'table' && item.__config__.children && Array.isArray(item.__config__.children)) {
    ...
    beforeSubmit() {
      ...
      const func = this.qt.getScriptFunc.call(this, this.formConfCopy.funcs.beforeSubmit)
    ...
    afterSubmit() {
      ...
      const func = this.qt.getScriptFunc.call(this, this.formConfCopy.funcs.afterSubmit)
      if (!func) return
      func(this.parameter)
    },
    submitForm() {
    ...
    onCascaderChange(data, on) {
      ...
      const func = this.qt.getScriptFunc.call(this, on.change)
      if (!func) return
      func.call(this, { data, ...this.parameter })
    },
    onCascaderBlur(data, on) {
      if (!on || !on.blur) return
      const func = this.qt.getScriptFunc.call(this, on.blur)
      if (!func) return
      func.call(this, { data, ...this.parameter })
    }
  },
