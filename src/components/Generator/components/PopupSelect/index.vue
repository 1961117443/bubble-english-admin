  ...
  <div class="popupSelect-container">
    ...
    <template v-if="popupType==='dialog'">
      <el-dialog :title="popupTitle" :close-on-click-modal="false" :visible.sync="visible"
        v-if="visible" class="QT-dialog QT-dialog_center" lock-scroll append-to-body
        :width='popupWidth'>
        <el-row class="QT-common-search-box" :gutter="16">
          <el-form @submit.native.prevent>
            <slot name="search" :listQuery="listQuery" :search="search">
              <el-col :span="10">
        ...
        <QT-table v-loading="listLoading" :data="list" :border="false" highlight-current-row
    ...
    <template v-if="popupType ==='drawer'">
      <el-drawer :title="popupTitle" :visible.sync="visible" :wrapperClosable="false" ref="drawer"
        :size='popupWidth' append-to-body class="QT-common-drawer">
...
export default {
  ...
  methods: {
    initData() {
      ...
      if (!this.interfaceId && !this.loadData) return
      ...
      let func
      if (this.interfaceId) {
        func = getDataInterfaceDataSelect(this.interfaceId, query)
      }else if(this.loadData){
        func = this.loadData(query)
      }
    ...
    interfaceDataHandler(data) {
      ...
      const dataHandler = this.qt.getScriptFunc.call(this, data.dataProcessing)
    ...
    setDefault() {
      if (this.value) {
        if (!this.interfaceId && !this.dataInfo) return
        let query = {
          id: this.value,
          interfaceId: this.interfaceId,
          propsValue: this.propsValue,
        ...
        const func = this.interfaceId ? getDataInterfaceDataInfo(this.interfaceId, query) : this.dataInfo(query)
        func.then(res => {
