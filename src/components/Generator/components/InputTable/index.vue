<template>
  <div class="qt-table-box" :class="[config.__config__.type]">
    <div class="QT-common-title" v-if="config.__config__.showTitle && config.__config__.label">
      <h2>{{ config.__config__.label }}</h2>
    </div>
    <el-table :data="tableFormData" class="QT-common-table" @cell-click="focusInput"
      ...
      <template v-for="(head, cindex) in tableData">
        <el-table-column :key="head.__config__.formId" :min-width="head['min-width']"
          ...
          <template slot-scope="scope">
            ...
            <template v-if="['select', 'checkbox','radio'].includes(head.__config__.qtKey)">
            ...
            <!-- 单行输入 -->
            <template v-else-if="head.__config__.qtKey==='comInput'">
  ...
  methods: {
    ...
    setTableFormData(prop, value) {
      ...
      for (let i = 0; i < activeRow.length; i++) {
        ...
        if (activeRow[i].__vModel__.indexOf('_qtRelation_') >= 0) {
          vModel = activeRow[i].__vModel__.substring(0, activeRow[i].__vModel__.indexOf('_qtRelation_'))
        }
        if (vModel === prop) {
    ...
    getTableFieldOptions(prop) {
      ...
      for (let i = 0; i < this.tableData.length; i++) {
        if (this.tableData[i].__vModel__ === prop) {
          ...
          let isTreeSelect = item.__config__.qtKey === 'treeSelect' || item.__config__.qtKey === 'cascader'
    ...
    onFormBlur(rowIndex, colIndex, tag) {
      ...
      if (data && data.on && data.on.blur) {
        const func = this.qt.getScriptFunc.call(this, data.on.blur)
        if (!func) return
        func.call(this, {
          data: null,
          ...this.parameter
    ...
    onFormDataChange(rowIndex, colIndex, tag, params) {
      ...
      if (data && data.on && data.on.change) {
        const func = this.qt.getScriptFunc.call(this, data.on.change)
        if (!func) return
        let value = ''
        if (['select', 'radio', 'checkbox'].includes(data.qtKey)) {
          const options = data.options
          if (data.config.multiple || data.qtKey === 'checkbox') {
            let _value = []
    ...
    getConfById(formId, rowIndex) {
      ...
      if (['relationForm', 'popupSelect'].includes(itemConfig.qtKey)) {
        newObj['field'] = item.__vModel__ + '_qtRelation_' + rowIndex
      }
      if (['relationFormAttr', 'popupAttr'].includes(itemConfig.qtKey)) {
        let prop = newObj['relationField'].split('_qtTable_')[0]
        newObj['relationField'] = prop + '_qtRelation_' + rowIndex
      }
      return newObj
    },
    /**
    ...
    getEmptyRow(val, rowIndex) {
      return this.tableData.map((t, index) => {
        let res = {
          ...
          regErrorText: '',
          on: t.on || {},
          qtKey: t.__config__.qtKey,
          __vModel__: ['relationForm', 'popupSelect'].includes(t.__config__.qtKey) ? t.__vModel__ + '_qtRelation_' + rowIndex : t.__vModel__,
    ...
    getTableValue() {
      return this.tableFormData.map(row => row.reduce((p, c) => {
        ...
        if (c.__vModel__ && c.__vModel__.indexOf('_qtRelation_') >= 0) {
          str = c.__vModel__.substring(0, c.__vModel__.indexOf('_qtRelation_'))
        }
        p[str] = c.value
