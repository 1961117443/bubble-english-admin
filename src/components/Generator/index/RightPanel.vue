        ...
        <el-form v-show="currentTab==='field' && showField" size="small" label-width="90px"
          ...
          <template v-if="activeData.__config__">
            <el-form-item label="控件类型">
              <el-select v-model="activeData.__config__.qtKey" placeholder="请选择控件类型" @change="componentChange">
                <el-option-group
            ...
            <template v-if="$store.getters.hasTable">
              <template v-if="activeData.__config__.qtKey==='table'">
                <el-form-item
                  v-if="activeData.__vModel__!==undefined && !noVModelList.includes(activeData.__config__.qtKey)"
                  label="控件字段">
            ...
            <QTComInput v-if="activeData.__config__.qtKey==='comInput'"
              :active-data="activeData" />
            <QT-textarea v-if="activeData.__config__.qtKey==='textarea'"
              :active-data="activeData" />
            <QTText v-if="activeData.__config__.qtKey==='QTText'" :active-data="activeData" />
            <QTNumInput v-if="activeData.__config__.qtKey==='numInput'"
              :active-data="activeData" />
            <QTAmount v-if="activeData.__config__.qtKey==='QTAmount'"
              :active-data="activeData" />
            <QTPsdInput v-if="activeData.__config__.qtKey==='PsdInput'"
              :active-data="activeData" />
            <QTRadio v-if="activeData.__config__.qtKey==='radio'" :active-data="activeData" />
            <QTCheckbox v-if="activeData.__config__.qtKey==='checkbox'"
              :active-data="activeData" />
            <QTSelect v-if="activeData.__config__.qtKey==='select'" :active-data="activeData"
              :key="activeData.__config__.renderKey" :dictionaryOptions="dictionaryOptions"
              :dataInterfaceOptions="dataInterfaceOptions" />
            <QTCascader v-if="activeData.__config__.qtKey==='cascader'"
              :active-data="activeData" />
            <QTTime v-if="activeData.__config__.qtKey==='time'" :active-data="activeData" />
            <QTTimeRange v-if="activeData.__config__.qtKey==='timeRange'"
              :active-data="activeData" />
            <QTDate v-if="activeData.__config__.qtKey==='date'" :active-data="activeData" />
            <QTDateRange v-if="activeData.__config__.qtKey==='dateRange'"
              :active-data="activeData" />
            <QTColorPicker v-if="activeData.__config__.qtKey==='colorPicker'"
              :active-data="activeData" />
            <QTRate v-if="activeData.__config__.qtKey==='rate'" :active-data="activeData" />
            <QTSwitch v-if="activeData.__config__.qtKey==='switch'" :active-data="activeData" />
            <QTSlider v-if="activeData.__config__.qtKey==='slider'" :active-data="activeData" />
            <QTDivider v-if="activeData.__config__.qtKey==='divider'"
              :active-data="activeData" />
            <QTUploadFz v-if="activeData.__config__.qtKey==='uploadFz'"
              :active-data="activeData" />
            <QTUploadImg v-if="activeData.__config__.qtKey==='uploadImg'"
              :active-data="activeData" />
            <QTComRight v-if="isCommon" :active-data="activeData" />
            <QTAddress v-if="activeData.__config__.qtKey==='address'"
              :active-data="activeData" />
            <TreeSelect v-if="activeData.__config__.qtKey==='treeSelect'"
              :active-data="activeData" />
            <GroupTitle v-if="activeData.__config__.qtKey==='groupTitle'"
              :active-data="activeData" />
            <RelationForm v-if="activeData.__config__.qtKey==='relationForm'"
              :active-data="activeData" v-on="$listeners" :key="activeData.__config__.renderKey" />
            <RelationFormAttr v-if="activeData.__config__.qtKey==='relationFormAttr'"
              :active-data="activeData" ref="relationFormAttr" />
            <QTCalculate v-if="activeData.__config__.qtKey==='calculate'"
              :active-data="activeData" ref="calculate" />
            <PopupSelect v-if="activeData.__config__.qtKey==='popupSelect'"
              :active-data="activeData" />
            <PopupAttr v-if="activeData.__config__.qtKey==='popupAttr'" :active-data="activeData"
              ref="popupAttr" />
            <Tab v-if="activeData.__config__.qtKey==='tab'" :active-data="activeData" />
            <Collapse v-if="activeData.__config__.qtKey==='collapse'" :active-data="activeData" />
            <template v-if="activeData.__config__.qtKey==='barcode'">
            ...
            <template v-if="activeData.__config__.qtKey==='qrcode'">
              <el-form-item label="控件标题">
                <el-input v-model="activeData.__config__.label" placeholder="请输入控件标题" />
              </el-form-item>
            ...
            <template v-if="activeData.__config__.qtKey==='button'">
            ...
            <template v-if="isSystem">
              ...
              <el-form-item label="选择模板" v-if="activeData.__config__.qtKey==='billRule'">
                <BillRule v-model="activeData.__config__.rule" placeholder="选择模板" />
              </el-form-item>
              <el-form-item label="显示内容" v-if="activeData.__config__.qtKey==='currOrganize'">
            ...
            <template v-if="activeData.__config__.qtKey==='card'">
            ...
            <!-- row 布局 -->
            <template v-if="activeData.__config__.qtKey==='row'">
          ...
          <template v-if="activeData.__config__">
            <template v-if="!activeData.__config__.isSubTable">
              ...
              <el-form-item label="标题宽度"
                v-if="!['divider','QTText','button','table','groupTitle ','card','row','tab','collapse'].includes(activeData.__config__.qtKey)">
          ...
          <template v-if="modelType!=3 && modelType!=6 && webType!=3">
            ...
            <template v-if="webType==2">
              ...
              <el-form-item label="" v-if="formConf.hasPrintBtn">
                <QT-TreeSelect :options="printTplList" v-model="formConf.printId"
                  placeholder="请选择打印模板" lastLevel clearable></QT-TreeSelect>
              </el-form-item>
            </template>
          </template>
...
import QTComInput from './RightComponents/ComInput'
import QTTextarea from './RightComponents/Textarea'
import QTText from './RightComponents/QTText'
import QTNumInput from './RightComponents/NumInput'
import QTAmount from './RightComponents/QTAmount'
import QTPsdInput from './RightComponents/PsdInput'
import QTRadio from './RightComponents/Radio'
import QTCheckbox from './RightComponents/Checkbox'
import QTSelect from './RightComponents/Select'
import QTCascader from './RightComponents/Cascader/index'
import QTTime from './RightComponents/Time'
import QTTimeRange from './RightComponents/TimeRange'
import QTDate from './RightComponents/Date'
import QTDateRange from './RightComponents/DateRange'
import QTColorPicker from './RightComponents/ColorPicker'
import QTRate from './RightComponents/Rate'
import QTSwitch from './RightComponents/Switch'
import QTSlider from './RightComponents/Slider'
import QTDivider from './RightComponents/Divider'
import QTUploadFz from './RightComponents/UploadFz'
import QTUploadImg from './RightComponents/UploadImg'
import QTComRight from './RightComponents/ComRight'
import QTAddress from './RightComponents/Address'
...
import QTCalculate from './RightComponents/Calculate'
...
export default {
  ...
  computed: {
    ...
    isCommon() {
      return commonRightList.indexOf(this.activeData.__config__.qtKey) > -1
    },
    isSystem() {
      return systemList.indexOf(this.activeData.__config__.qtKey) > -1
    },
    drawingOptions() {
      let list = []
      const loop = (data, parent) => {
        ...
        if (data.__vModel__ && data.__config__.qtKey !== 'table') {
          list.push(data)
        }
      }
  ...
  created() {
    ...
    if (!this.activeData.__config__.visibility) this.$set(this.activeData.__config__, "visibility", ["pc", "app"])
    if (!this.activeData.__config__.tableName && this.activeData.__config__.qtKey !== 'table') {
  ...
  methods: {
    ...
    fieldChange1(val) {
      ...
      const loop = list => {
        for (let i = 0; i < list.length; i++) {
          ...
          if (config.qtKey === 'table' && config.tableName === this.activeData.__config__.relationTable) {
          ...
          if (config && config.qtKey != 'table' && config.children && Array.isArray(config.children)) {
    ...
    setDefaultOptions() {
      ...
      if (this.activeData.__vModel__ === undefined || this.activeData.__config__.qtKey === 'table') return
      if (!this.activeData.__config__.tableName || this.activeData.__config__.tableName === this.mainTable) {
        ...
        if (!list.length) {
          ...
          let options = item.fields.map(o => ({
            ...o,
            realField: 'qt_' + this.activeData.__config__.tableName + '_qt_' + o.field,
          }))
    ...
    onTableNameChange(tableName) {
      ...
      const loop = list => {
        for (let i = 0; i < list.length; i++) {
          const e = list[i]
          const config = e.__config__
          if (config.qtKey === 'table' && config.tableName === tableName) {
            boo = true
            break
          }
          if (config && config.qtKey != 'table' && config.children && Array.isArray(config.children)) {
    ...
    componentChange(e){
      console.log('当前的字段类型：', this.activeData.__config__)
      let component ;
      for (let i = 0; i < this.leftComponents.length; i++) {
        const group = this.leftComponents[i];
        component = group.list.find(x=>x.__config__.qtKey == e)
        if (component) {
          break
        }
      }
      console.log('切换后的字段类型：', component.__config__)

      this.$emit('comChange',this.activeData, component)
    }
  }
}
</script>
