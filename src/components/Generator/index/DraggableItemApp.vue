...
const layouts = {
  colFormItem(h, element, index, parent) {
    ...
    if (config.qtKey === 'barcode') {
      return (
        <el-col span={24} class={className}
          nativeOnClick={event => { activeItem(element); event.stopPropagation() }}>
          <el-form-item label-width={labelWidth}
            label={config.showLabel ? config.label : ''} required={config.required}>
            <qt-barcode format={element.format} lineColor={element.lineColor} background={element.background} width={element.width} height={element.height} staticText={element.staticText}></qt-barcode>
          </el-form-item>
          {components.itemBtns.apply(this, arguments)}
        </el-col>
      )
    }
    if (config.qtKey === 'qrcode') {
      return (
        <el-col span={24} class={className}
          nativeOnClick={event => { activeItem(element); event.stopPropagation() }}>
          <el-form-item label-width={labelWidth}
            label={config.showLabel ? config.label : ''} required={config.required}>
            <qt-qrcode size={element.size} colorLight={element.colorLight} colorDark={element.colorDark} staticText={element.staticText}></qt-qrcode>
          </el-form-item>
          {components.itemBtns.apply(this, arguments)}
        </el-col>
      )
    }
    if (config.qtKey === 'button') {
      return (
        <el-col span={24} class={className}
          nativeOnClick={event => { activeItem(element); event.stopPropagation() }}>
          <el-form-item label-width="0">
            <qt-button align={element.align} buttonText={element.buttonText} type={element.type} disabled={element.disabled}></qt-button>
          </el-form-item>
          {components.itemBtns.apply(this, arguments)}
        </el-col>
      )
  ...
  rowFormItem(h, element, index, parent) {
    ...
    const className = this.activeId === element.__config__.formId
      ...
      : 'drawing-row-item'
    if (element.__config__.qtKey === 'tab') {
      return (
        <el-col span={24}>
    ...
    if (element.__config__.qtKey === 'collapse') {
      return (
        <el-col span={24}>
          <el-row gutter={element.__config__.gutter} class={className}
            nativeOnClick={event => { activeItem(element); event.stopPropagation() }}>
    ...
    if (element.__config__.qtKey === 'row') {
    ...
    if (element.__config__.qtKey === 'card') {
      let tip = ''
      if (!element.__config__.children.length) {
        tip = <div class="table-tip">请将组件拖到此区域(可拖多个组件)</div>
      }
    ...
    if (element.__config__.qtKey === 'table') {
      let tip = ''
      if (!element.__config__.children.length) {
        tip = <div class="table-tip">请将组件拖到此区域(可拖多个组件)</div>
      }
