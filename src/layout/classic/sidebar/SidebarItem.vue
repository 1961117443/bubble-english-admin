<template>
  <div>
    <el-submenu ref="subMenu" :index="item.path" popper-append-to-body
      ...
      <template slot="title">
        <item :icon="item.icon" :title="generateTitle(item.vueName,item.fullName)" :id="item.id" />
      </template>
    ...
    <template v-else>
      ...
      <template v-else>
        <app-link :to="item.path" :item="item" v-if="item.path && checkProperty(item)">
          <el-menu-item :index="item.path" :class="{'submenu-title-noDropdown':!isNest}">
            <item :icon="item.icon" :title="generateTitle(item.vueName,item.fullName)" :id="item.id" />
          </el-menu-item>
        </app-link>
