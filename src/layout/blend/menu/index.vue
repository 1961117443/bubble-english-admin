  ...
  methods: {
    setDefault() {
      ...
      if (currPath === '/home' || currPath === '/dashboard') {
        ...
        const blendId = this.qt.storageGet('blend_id') || ''
        if (blendId && this.menuList.length) {
          const menu = this.menuList.find(x=>x.id == blendId)
          if (menu && menu.type === 1) {
            this.activeName = menu.path
            this.$store.commit('user/SET_LEFTMENULIST', menu.children || [])
            return
          }
        }
