  ...
  <div class="im-container" v-if="visible">
    ...
    <div class="main">
        ...
        <div class="toolBox">
          <div class="toolBox-left">
            <el-popover placement="top-start" trigger="click" ref="popover"
              ...
              <el-link icon="qt-custom qt-custom-emoticon-neutral" :underline="false" title="发送表情"
                slot="reference">
              </el-link>
            </el-popover>
            <el-upload :show-file-list="false" :action="define.comUploadUrl+'/IM'" :limit="1"
              ...
              accept="image/*">
              <el-link icon="qt-custom qt-custom-image" :underline="false" title="发送图片">
              </el-link>
            </el-upload>
          </div>
          <div class="toolBox-right">
            <el-link icon="icon-qt icon-qt-generator-time" :underline="false" @click="openHistory">
              聊天记录
            </el-link>
          </div>
        </div>
      ...
      <div class="historyBox" v-if="showHistory">
        <el-input v-model="keyword" placeholder="搜索：请输入关键词" class="QT-common-head-input" clearable
    ...
    <el-dialog :visible.sync="show" width="40%" :before-close="handleClose"
      class="QT-dialog QT-dialog_center" lock-scroll style="z-index:3000">
    ...
    getList(data) {
      ...
      for (let i = 0; i < data.list.length; i++) {
        ...
        list.push({
          ...
          dateTime: this.qt.toDate(item.sendTime)
        })
      }
      if (!this.showHistory) {
        this.list = list
