        ...
        <template v-for="route in menuList">
          <el-submenu ref="subMenu" :index="route.path" popper-append-to-body
            ...
            <template slot="title">
              ...
              <div class="mainTitle">                
                {{generateTitle(route.vueName,route.fullName)}}
                <template v-if="remindData[route.id]">
                  <el-badge v-if="remindData[route.id].isDot" is-dot />
                  <el-badge v-else :value="remindData[route.id].num" />
                </template>
              </div>
          ...
          <template v-else>
            <app-link :to="route.path" :key="route.path">
              <el-menu-item :index="route.path" :class="{'submenu-title-noDropdown':true}">
                <item :icon="route.icon"  :id="route.id" />
...
export default {
  ...
  computed: {
    ...
    ...mapState({
      ...
      remindData: state => state.user.remindData
    }),
    classObj() {
      return {
