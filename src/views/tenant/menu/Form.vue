<template>
  <el-dialog :title="!dataForm.id ? '新建菜单' : '编辑菜单'" :close-on-click-modal="false"
    ...
    class="QT-dialog QT-dialog_center" width="600px">
    <el-form ref="dataForm" :model="dataForm" :rules="dataRule" label-width="100px"
      ...
      <el-form-item label="上级" prop="parentId">
        <QT-TreeSelect v-model="dataForm.parentId" :options="treeData" placeholder="选择上级菜单"
          @change="onChange" />
      </el-form-item>
      ...
      <el-form-item label="图标" prop="icon">
        <el-row type="flex">
          ...
          <el-color-picker v-model="dataForm.propertyJson.iconBackgroundColor"
            ...
            popper-class="qt-color-picker-class" />
        </el-row>
      </el-form-item>
      <el-form-item label="类型" prop="type" v-show="false">
        <el-select v-model="dataForm.type" placeholder="请选择类型" @change="changeMenuType" disabled>
          <el-option v-for="item in typeData" :key="item.enCode" :label="item.fullName"
            :value="item.enCode">
          </el-option>
        </el-select>
      </el-form-item>
      <el-form-item v-if="dataForm.type == 2 || dataForm.type == 7" label="地址" prop="urlAddress" v-show="false">
        <el-input v-model="dataForm.urlAddress" placeholder="填写地址" disabled>
      ...
      <el-form-item v-if="[3,4,5,6,8].indexOf(dataForm.type)>-1" label="关联"
        ...
        <QT-TreeSelect v-model="dataForm.propertyJson.moduleId" :options="tempData"
...
import iconBox from '@/components/QT-iconBox'
const appTypeData = [{
  enCode: 1,
  fullName: "目录"
}, {
