<template>
  <el-dialog title="数据选择" :close-on-click-modal="false" width="700px"
    class="QT-dialog QT-dialog_center table-dialog" lock-scroll append-to-body v-bind="$attrs"
    ...
    <QT-table v-loading="listLoading" :data="list" @selection-change="handleSelectionChange"
...
<script>
import { DataModelList,DataModelViewList } from '@/api/systemData/dataModel'
export default {
  props: ['dbLinkId'],
  data() {
    return {
  ...
  methods: {
    ...
    initData() {
      ...
      DataModelViewList(dbLinkId, { keyword: this.keyword }).then(res => {
