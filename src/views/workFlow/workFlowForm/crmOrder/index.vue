      ...
      <el-row>
        ...
        <el-col :span="24" v-if="judgeShow('fileJson')">
          <el-form-item label="相关附件" prop="fileJson">
            <QT-UploadFz v-model="fileList" type="workFlow" :disabled="judgeWrite('fileJson')" />
          </el-form-item>
        </el-col>
...
import GoodsBox from '@/views/example/order/GoodsBox'
...
export default {
  ...
  methods: {
    ...
    count(row) {
      //金额 = 数量*单价
      row.amount = this.qt.toDecimal(parseFloat(row.price) * parseFloat(row.qty))
      //折扣价 = (单价*折扣)
      var discountPrice = row.price * (row.discount ? row.discount / 100 : 1);
      //实际单价 = 折扣价 * (1 + (税率 / 100))
      row.actualPrice = this.qt.toDecimal(discountPrice * (1 + (row.cess ? row.cess / 100 : 0)));
      //实际金额
      row.actualAmount = this.qt.toDecimal(parseFloat(row.actualPrice) * parseFloat(row.qty))
    }
  }
}
</script>
