<template>
  <transition name="el-zoom-in-center">
    <div class="QT-preview-main user-form">
      ...
      <div class="main" v-loading="formLoading">
        <el-form ref="dataForm" :model="dataForm" :rules="dataRule" label-width="100px">
          <div class="QT-common-title mb-20">
...
export default {
  ...
  methods: {
    ...
    async  onOrganizeChange(val) {
    //  console.log(this.dataForm.roleId,this.roleId)
      let lastRoleId = this.roleId || []
      ...
      this.formLoading = true
      try {
        await this.getOptionsByOrgIds(val)
      }finally{
        this.formLoading = false
      }
      // console.log(this.roleTreeData)
      if (lastRoleId && lastRoleId.length) {
        // 判断当前组织是否包含之前选择的角色
        for (let i = lastRoleId.length-1; i >-1; i--) {
          if ( this.roleTreeData.filter(x=>x.children && x.children.length).some(o => o.children.some(z=>z.id == lastRoleId[i]) )) {
            continue
          }else{
            lastRoleId.splice(i,1)
          }
        }
        if (lastRoleId.length) {
          this.roleId = lastRoleId
          this.dataForm.roleId = lastRoleId.join(",")
        }
      }
    },
    async getOptionsByOrgIds(organizeIdTree) {
      const organizeIds = organizeIdTree.map(o => o[o.length - 1])
      this.dataForm.organizeId = organizeIds.join()
      await getPositionByOrganize(organizeIds).then(res => {
