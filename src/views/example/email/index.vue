  ...
  <div class="app-container Email-container nohead">
    <el-tabs tab-position="left" style="height:100%" v-model="activeTab"
      ...
      <el-tab-pane name="writeMail">
        <span slot="label"><i class="icon-qt icon-qt-btn-edit"></i>写邮件</span>
      </el-tab-pane>
      <el-tab-pane name="inBox">
        <span slot="label"><i class="icon-qt icon-qt-extend-envelope"></i>收件箱</span>
      </el-tab-pane>
      <el-tab-pane name="star">
        <span slot="label"><i class="icon-qt icon-qt-extend-star"></i>星标件</span>
      </el-tab-pane>
      <el-tab-pane name="draft">
        <span slot="label"><i class="icon-qt icon-qt-extend-exclamation-triangle"></i>草稿箱</span>
      </el-tab-pane>
      <el-tab-pane name="sent">
        <span slot="label"><i class="icon-qt icon-qt-extend-paper-plane"></i>已发送</span>
      </el-tab-pane>
      <el-tab-pane name="setAccount">
        <span slot="label"><i class="icon-qt icon-qt-extend-cog"></i>邮箱配置</span>
      </el-tab-pane>
            ...
            <template slot-scope="scope">
              <template v-if="activeTab==='inBox'|| activeTab==='star'">
                <span v-if=" scope.row.isRead">
                  <i class="icon-qt icon-qt-extend-envelope-open-o i-default" title='点击标记为未读'
                    @click="ReceiveUnread(scope.row)"></i></span>
                <span v-else><i class="icon-qt icon-qt-extend-envelope text-warning" title='点击标记为已读'
                    @click="ReceiveRead(scope.row)"></i></span>
              </template>
              <span v-else><i class="icon-qt icon-qt-extend-envelope-open-o i-default"></i></span>
            </template>
          </el-table-column>
          <el-table-column prop="attachment" label="" width="40">
            <template slot-scope="scope">
              <span v-if="scope.row.attachment && JSON.parse(scope.row.attachment).length"><i
                  class="icon-qt icon-qt-extend-paperclip i-default"></i></span>
            </template>
          </el-table-column>
          ...
          <el-table-column prop="starred" label="" width="40"
            ...
            <template slot-scope="scope">
              <span v-if="scope.row.starred"><i
                  class="icon-qt icon-qt-extend-star text-warning img-star" title='点击取消星标'
                  @click="ReceiveNoStarred(scope.row,scope.$index)"></i></span>
              <span v-else><i class="icon-qt icon-qt-extend-star-o i-default img-star"
