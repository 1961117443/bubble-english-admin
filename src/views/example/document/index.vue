  ...
  <div class="app-container Document-container nohead">
    <el-tabs tab-position="left" style="height:100%" v-model="activeTab">
      <el-tab-pane name="allPanel">
        <span slot="label"><i class="icon-qt icon-qt-extend-folder-open"></i>全部文档</span>
        <div class="main QT-flex-main">
          ...
          <QT-table v-loading="listLoading" :data="list" empty-text="该文件夹为空" size="mini">
            <el-table-column prop="fullName" label="文件名">
              <template slot-scope="scope">
                <span v-if="scope.row.type">
                  <i :class='"icon-qt " + toFileExt(scope.row.fileExtension) + " i-default"' />
                  {{scope.row.fullName}}
                </span>
                <span class='cursor-pointer' v-else @click="openFolder(scope.row)">
                  <i class='icon-qt icon-qt-extend-folder text-warning' />
                  {{scope.row.fullName}}
                </span>
              </template>
            </el-table-column>
            <el-table-column prop="isShare" label="" width="35">
              <template slot-scope="scope">
                <span v-if="scope.row.type && scope.row.isShare" title='共享文件'><i
                    class="icon-qt icon-qt-extend-thumbs-up i-default"></i></span>
              </template>
            </el-table-column>
            <el-table-column prop="fileSize" label="大小" width="90">
              <template slot-scope="scope">{{scope.row.fileSize | toFileSize()}}</template>
            </el-table-column>
            <el-table-column prop="creatorTime" label="创建日期" :formatter="qt.tableDateFormat"
              width="120" />
      ...
      <el-tab-pane name="shareoutPanel">
        <span slot="label"><i class="icon-qt icon-qt-extend-thumbs-up"></i>我的共享</span>
        <div class="main QT-flex-main">
          <div class="QT-common-head">
            <el-breadcrumb>
          ...
          <QT-table v-loading="listLoading" :data="list" empty-text="该文件夹为空" size="mini">
            ...
            <el-table-column prop="shareTime" label="共享日期" :formatter="qt.tableDateFormat"
              width="120" />
      ...
      <el-tab-pane name="sharetomePanel">
        <span slot="label"><i class="icon-qt icon-qt-extend-share"></i>共享给我</span>
        <div class="main QT-flex-main">
          <div class="QT-common-head">
            <el-breadcrumb>
      ...
      <el-tab-pane name="trashPanel">
        <span slot="label"><i class="icon-qt icon-qt-extend-trash"></i>回收站</span>
        <div class="main QT-flex-main">
          <div class="QT-common-head">
            <el-breadcrumb>
          ...
          <QT-table v-loading="listLoading" :data="list" empty-text="该文件夹为空" size="mini">
            ...
            <el-table-column prop="deleteTime" label="删除日期" :formatter="qt.tableDateFormat"
              width="120" />
  ...
  methods: {
    ...
    handleDownLoad(id) {
      Download(id).then(res => {
        this.qt.downloadFile(res.data.url, res.data.name)
      })
    },
    // 打开文件夹
    openFolder(item) {
    ...
    toFileExt(ext) {
      ...
      if (ext == "doc" || ext == "docx") {
        return "icon-qt-file-word";
      }
      if (ext == "xls" || ext == "xlsx") {
        return "icon-qt-file-excel";
      }
      if (ext == "ppt" || ext == "pptx") {
        return "icon-qt-file-ppt";
      }
      if (ext == "rar" || ext == "zip") {
        return "icon-qt-file-zip";
      }
      if (ext == "txt" || ext == "log") {
        return "icon-qt-file-text";
      }
      if (ext == "html" || ext == "cs" || ext == "xml") {
        return "icon-qt-file-code";
      }
      if (ext == "gif" || ext == "jpg" || ext == "jpeg" || ext == "bmp" || ext == "png") {
        return "icon-qt-file-image1";
      }
      if (ext == "pdf") {
        return "icon-qt-file-pdf";
      }
      if (ext == "mp3") {
        return "icon-qt-file-audio";
      }
      if (ext == "mp4") {
        return "icon-qt-file-movie";
      }
      return "icon-qt-file-blank";
    },
  }
}
</script>
