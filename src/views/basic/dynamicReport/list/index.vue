...
<template>
  <div class="QT-common-layout">
    <div class="QT-common-layout-left" v-if="columnData.type === 2">
      <div class="QT-common-title" v-if="columnData.treeTitle">
      ...
      <el-tree :data="treeData" :props="treeProps" default-expand-all highlight-current
        ...
        class="QT-common-el-tree" :node-key="treeProps.value">
    ...
    <div class="QT-common-layout-center" v-loading="listLoading" >
      <Search ref="Search" :list="columnData.searchList" @reset="reset" @search="search" @export="download" />
      <div class="QT-common-layout-main QT-flex-main">
        <div class="QT-common-head" v-if="false">
        ...
        <QT-table :data="list" row-key="id" default-expand-all
          ref="QTTableRef"
          :tree-props="{children: 'children', hasChildren: ''}" 
          :hasC="hasBatchBtn" :selectedRows="multipleSelection" 
            :show-summary="summaryColumnList.length>0" :summary-method="e=>qt.tableSummaries({data:list,columns:e.columns,summary:tableDataSummary},summaryColumnList)"
          custom-column border :tableKey="`QT-table-virtual_${modelId}`">
          <!-- :sortable="item.sortable?'custom':item.sortable" -->
          <!-- (a,b)=>sortMethod(item,a,b) -->
          <template #actions>
            <div class="QT-common-head" >
              <span v-if="hasBatchBtn">{{ selectedSummaryText }}</span>
            </div>            
          </template>
          <el-table-column :prop="item.prop" :label="item.label" :align="item.align"
            :width="item.width" :key="i" sortable
              ...
              <template v-if="isPreview || !columnData.useBtnPermission">
                <template v-for="(item, i) in columnBtnsList">
                  ...
                  <template v-else-if="item.value=='remove'">
                    <el-button size="mini" type="text" :key="i" class="QT-table-delBtn"
        ...
        <template v-if="columnData.type !== 3 && columnData.hasPage && list.length !=total">
    ...
    <EditColValueDialogVue v-if="editColValueDialogVisible" ref="EditColValueDialogRef" @ok="editColValueDialogVisible=false"></EditColValueDialogVue>
  </div>
</template>
...
import QTTableVirtual from '@/components/QT-table-virtual'
import EditColValueDialogVue from './EditColValueDialog.vue'
export default {
  name: 'dynamicReportIndex',
  components: { Form, ExportBox, Search, Detail, FlowBox,'QT-table-virtual':QTTableVirtual,EditColValueDialogVue },
  ...
  data() {
    return {
      ...
      multipleSelection: [],
      settingsColumnList: [],
      editColValueDialogVisible:false,
      tableDataSummary:{}
    }
  },
  computed: {
    ...
    summaryColumnList(){
      ...
      return this.columnData.columnList.filter(o=>!!o.summary).map(o=>o.prop) || []

    },
    selectedSummaryText(){
      const desc = []
      if(this.summaryColumnList && this.multipleSelection){
        this.summaryColumnList.forEach(c=>{
          const col = this.columnData.columnList.find(x=>x.prop == c)
          // console.log(c,this.qt.sum(this.multipleSelection,c))
          if(col){
            desc.push(`${col.label}：${this.qt.sum(this.multipleSelection,c)}`)
          }
          
        })
      }
      return `当前选中行合计：${desc.join('，')}`
    }
  },
  created() {
    this.init()
  ...
  methods: {
    async init() {
      ...
      this.columnData.columnList = this.columnData.columnList.filter(o=>!o.hidden)
      // console.log(this.columnData.columnList)
      ...
      // 设置查询字段
      // console.log(deepClone(this.columnData.searchList))
      if (!this.columnData.searchList.length) {
        this.columnData.searchList = this.columnData.columnList.filter(o=>o.query).map(x=>{
          let o= deepClone(x)
          o.__config__.label = x.label
          o.__vModel__ = x.prop
          o.value = ''
          return o
        })
      }else{
        this.columnData.searchList =this.columnData.searchList.map(x=>{
          let o= deepClone(x)
          o.__config__.label = x.label
          o.__vModel__ = x.prop
          o.value = ''
      ...
      // console.log(this.columnData.searchList)
      ...
      // 获取数据字典
      await this.getDictionaryDataSelector()
    ...
    initData() {
      ...
      getModelList(this.modelId, this.listQuery).then(res => {
        ...
        const {tableDataSummary} = res.extras ||{}
        if (tableDataSummary) {
    ...
    customBtnsHandel(item, row, index) {
      ...
      const func = this.qt.getScriptFunc.call(this, item.func)
      if (!func) return
    ...
    async getDictionaryDataSelector(){
      var list = this.columnList.filter(x=>x.__config__ && x.__config__.dataType === 'dictionary' && x.__config__.dictionaryType)
    // console.log(  this)
      this.listLoading = true
      for(var i = 0; i < list.length; i++){
        const item = list[i]
        const val = item.__config__.dictionaryType
        await getDictionaryDataSelector(val).then(res => {
          if (!Object.hasOwnProperty.call(item, "__slot__")) {
            this.$set(item, "__slot__", {});
          }
          if (!Object.hasOwnProperty.call(item.__slot__, "options")) {
            this.$set(item.__slot__, "options", []);
          }
          this.$set(item.__slot__, "options",  res.data.list || []);
          // item.__slot__.options = res.data.list
          item.formatter = (row, column, cellValue, index)=>dynamicText(cellValue,item.__slot__.options,item.__config__.props.value,item.__config__.props.label)
          // this.$set(item,'formatter',(row, column, cellValue, index)=>{
          //   return dynamicText(cellValue,item.__slot__.options,item.__config__.props.value,item.__config__.props.label)
          //   // return 123
          // })
        })
      }
    ...
    sortMethod(column,a,b){
      console.log(column,a,b)
      return Array.sort(a,b)
    },
    setEditableColumn(){
      var list = this.columnList.filter(x=>x.editable == true)
      for(var i = 0; i < list.length; i++){
        const item = list[i]
        ...
        item.formatter = (row, column, cellValue, index)=> {
          return (
            <span>
              ...
              <el-button type="text" size="medium" style="padding:0px" icon="el-icon-edit" onClick={()=>this.handleColumnEdit(row, column, cellValue,item)}>
              </el-button>
              {cellValue}
            </span>
            )
      ...
      if (!this.hasBatchBtn && list.length>0) {
        this.$nextTick(()=>{
          this.hasBatchBtn = true
        })
      }
    },
    handleColumnEdit(row, column, cellValue,config){
      // console.log('handleColumnEdit',row, column, cellValue,config)
      this.editColValueDialogVisible = true;
      this.$nextTick(()=>{
        this.$refs.EditColValueDialogRef.init({row, column, cellValue,config,$vue:this})      
      })
    }
  }
}
