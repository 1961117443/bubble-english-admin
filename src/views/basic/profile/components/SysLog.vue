<template>
  <div class="sysLog QT-flex-main">
    ...
    <el-tabs v-model="activeName" @tab-click="handleTabClick" class="QT-el_tabs">
      <el-tab-pane label="登录日志" name="1">
        <QT-table v-loading="listLoading" :data="loginLogData" tooltip-effect="dark">
          <el-table-column prop="creatorTime" label="登录时间" :formatter="qt.tableDateFormat"
            width="120" />
      ...
      <el-tab-pane label="请求日志" name="5">
        <QT-table v-loading="listLoading" :data="requestLogData" tooltip-effect="dark">
          <el-table-column prop="creatorTime" label="请求时间" :formatter="qt.tableDateFormat"
            width="120" />
  ...
  methods: {
    ...
    search() {
      if (this.pickerVal && this.pickerVal.length) {
        this.startTime = this.qt.toDate(this.pickerVal[0],'yyyy-MM-dd')
        this.endTime = this.qt.toDate(this.pickerVal[1],'yyyy-MM-dd')
      } else {
        this.startTime = ''
        this.endTime = ''
      }
