<template>
  <transition name="el-zoom-in-center">
    <div class="QT-preview-main flow-form-main">
      ...
      <el-row class="main">
        <el-col :span="14" :offset="5">
          <el-form :model="dataForm" :rules="dataRule" ref="dataForm" class="mt-20"
            ...
            <el-col :span="24">
              <el-form-item label="时间表达式" prop="executeContent.cron">
                <el-input v-model="dataForm.executeContent.cron" placeholder="时间表达式" readonly>
                  <el-button slot="append" icon="el-icon-edit-outline" @click="showDialog">
                  </el-button>
                </el-input>
              </el-form-item>
      ...
      <!-- <el-dialog title="Cron表达式" :visible.sync="showCron" :close-on-click-modal="false"
        class="QT-dialog QT-dialog_center" lock-scroll append-to-body width="700px"
      ...
      <el-dialog title="时间表达式" :visible.sync="showCron" :close-on-click-modal="false"
        class="QT-dialog QT-dialog_center" lock-scroll append-to-body width="1000px" @closed="showCrontab = false">
          <Cron v-if="showCron" ref="cronRef" :expression="exp" @CronChange="handelCron" />
          <CrontabResult v-if="showCron" :ex="exp" :times="10" />
        <span slot="footer" class="dialog-footer">
          <el-button @click="showCron = false">{{ $t('common.cancelButton') }}
          </el-button>
          <el-button type="warning" @click="resetCrontab">重 置</el-button>
...
import Cron from '@/components/TaskPlansCron'
import CrontabResult from '@/components/TaskPlansCron/Crontab-Result.vue'
export default {
  components: { vcrontab, InterfaceDialog,Cron,CrontabResult },
  data() {
    return {
      showCron: false,
      showCrontab: true,
  ...
  methods: {
    ...
    onLocalHostTaskIdChange(val) {
      ...
      if (!this.dataForm.enCode) {
        this.dataForm.enCode = item.id.replace('/','.')
      }
    },
    getCrontabValue() {
      this.$refs.vcrontab && this.$refs.vcrontab.submitFill()

      this.dataForm.executeContent.cron = this.exp
    ...
    resetCrontab() {
      ...
      this.exp = '0 /1 * * * ? *'
      this.$nextTick(()=>{
        this.$refs.cronRef.resolveExpression()
      })
    },
    crontabFill(value) {
    ...
    showDialog() {
      ...
      this.exp = this.dataForm.executeContent.cron
    },
    handelCron(val){
      // console.log('handelCron:',val,this.$refs.cronRef)
      this.exp = val
      // console.log(val)
      this.$nextTick(()=>{
        // this.$refs.cronRef.resolveExpression(val)
      })
    }
  }
}
