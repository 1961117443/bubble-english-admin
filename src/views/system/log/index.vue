<template>
  <div class="QT-common-layout systemLogs">
      ...
      <div class="QT-common-layout-main QT-flex-main">
        <el-tabs v-model="activeName" type="border-card" class="logTabs"
          ...
          <el-tab-pane label="登录日志" name="1">
            <QT-table v-loading="listLoading" :data="loginLogData" has-c
              @selection-change="handleSelectionChange">
              <el-table-column prop="creatorTime" :formatter="qt.tableDateFormat" label="登录时间"
                width="120" />
          ...
          <el-tab-pane label="请求日志" name="5">
            <QT-table v-loading="listLoading" :data="requestLogData" has-c
              @selection-change="handleSelectionChange">
              <el-table-column prop="creatorTime" :formatter="qt.tableDateFormat" label="请求时间"
                width="120" />
          ...
          <el-tab-pane label="操作日志" name="3">
            <QT-table v-loading="listLoading" :data="operationLogData" has-c
              @selection-change="handleSelectionChange">
              <el-table-column prop="creatorTime" :formatter="qt.tableDateFormat" label="操作时间"
                width="120" />
          ...
          <el-tab-pane label="异常日志" name="4">
            <QT-table v-loading="listLoading" :data="errorLogData" has-c
              @selection-change="handleSelectionChange">
          ...
          <el-tab-pane label="差异日志" name="6">
            <QT-table v-loading="listLoading" :data="diffLogData" has-c
              @selection-change="handleSelectionChange">
              <el-table-column prop="diffType" label="差异操作" width="80" align="center" />
              <el-table-column prop="sql" label="Sql语句" >
                <template slot-scope="scope">
                  <el-link @click="goDetail(scope.row.sql,'Sql语句')" style="font-size:12px">
                    <p class="line1">{{ scope.row.sql }}</p>
                  </el-link>
                </template>
              </el-table-column>
              <el-table-column prop="parameters" label="参数" >
                <template slot-scope="scope">
                  <el-link @click="goDetail(scope.row.parameters,'参数')" style="font-size:12px">
                    <p class="line1">{{ scope.row.parameters }}</p>
                  </el-link>
                </template>
              </el-table-column>
              <el-table-column prop="elapsed" label="耗时(ms)" width="80" align="center" />
              <!-- <el-table-column prop="Parameters" label="日志消息" width="120" /> -->
              <el-table-column prop="beforeData" label="操作前消息"  >
                <template slot-scope="scope">
                  <el-link @click="goDetail(scope.row.beforeData,'操作前消息')" style="font-size:12px">
                    <p class="line1">{{ scope.row.beforeData }}</p>
                  </el-link>
                </template>
              </el-table-column>
              <el-table-column prop="afterData" label="操作后消息">
                <template slot-scope="scope">
                  <el-link @click="goDetail(scope.row.afterData,'操作后消息')" style="font-size:12px">
                    <p class="line1">{{ scope.row.afterData }}</p>
                  </el-link>
                </template>
              </el-table-column>
              <el-table-column prop="businessData" label="业务对象" >
                <template slot-scope="scope">
                  <el-link @click="goDetail(scope.row.businessData,'业务对象')" style="font-size:12px">
                    <p class="line1">{{ scope.row.businessData }}</p>
                  </el-link>
                </template>
              </el-table-column>
              <el-table-column prop="createTime" label="操作时间" width="120" :formatter="qt.tableDateFormat" />
              <el-table-column prop="createUserIdName" label="操作人" width="120" />
            </QT-table>
          </el-tab-pane>
  ...
  methods: {
    initData() {
      ...
      getLogList(activeId, this.listQuery).then(res => {
        ...
        if (activeId === '6') this.diffLogData = res.data.list
    ...
    handleDel() {
      ...
      this.$confirm('您确定要删除这些数据吗, 是否继续?', '提示', {
        ...
        const req = this.activeName =='6' ? this.$request({
          url: '/api/system/Log/deleteBy/6',
          method: 'DELETE',
          data
        }) : delLog(data)
        req.then(res => {
