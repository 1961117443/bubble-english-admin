<template>
  <div class="QT-common-layout">
    <div class="QT-common-layout-center">
        ...
        <el-form @submit.native.prevent>
          ...
          <el-col :span="6">
            <el-form-item label="模块">
              <el-input v-model="moduleName" placeholder="请输入模块" clearable
                @keyup.enter.native="search()" />
            </el-form-item>
          </el-col>
          <el-col :span="6">
      ...
      <div class="QT-common-layout-main QT-flex-main">
        ...
        <QT-table v-loading="listLoading" :data="list">
          ...
          <el-table-column prop="creatorUser" label="创建人" width="200" />
          ...
          <el-table-column prop="moduleName" label="绑定模块" width="150">
            <template slot-scope="scope">
              <template  v-if="scope.row.moduleName && scope.row.moduleName.length > 0">
                <el-tag v-for="(item,index) in scope.row.moduleName" :key="index" disable-transitions>{{item}}</el-tag>
              </template>
            </template>
          </el-table-column>
              ...
              <tableOpts @edit="addOrUpdateHandle(scope.row.id)" @del="handleDel(scope.row.id)">
                <el-dropdown>
                  ...
                  <el-dropdown-menu slot="dropdown">
                    ...
                    <el-dropdown-item @click.native="$refs.bindFormRef.showDialog(scope.row.id)">绑定</el-dropdown-item>
                  </el-dropdown-menu>
                </el-dropdown>
              </tableOpts>
            </template>
    ...
    <BindForm ref="bindFormRef" @ok="search()" />
  </div>
</template>
...
export default {
  ...
  components: { Form, Preview,BindForm },
  data() {
    return {
      list: [],
      categoryList: [],
