  ...
  <el-dialog :visible.sync="visible" fullscreen lock-scroll class="QT-full-dialog"
    ...
    <div class="main" v-loading="loading">
        ...
        <el-col :xs="12" :sm="12" :md="12" :lg="12" :xl="10" class="basicForm">
          <el-form :model="dataForm" :rules="dataRule" ref="dataForm" label-width="100px"
            ...
            <el-row>
              ...
              <el-col :span="12">
                <el-form-item label="打印模式" prop="propertyJson.type">
                  <el-radio-group v-model="dataForm.propertyJson.type">
            ...
            <div class="mb-20">
              <el-alert title="默认打印的时候必须传业务主键@formId给SQL语句里面作为Where查询条件。如果需要传入自定义参数，需要用{}进行包裹，默认会取userInfo的值，例如{userId}" type="warning" show-icon
      ...
      <template v-if="activeStep==1">
        ...
        <hi-designer :treeData="treeData" :sysData="sysData" v-model="dataForm.printTemplate" :type="dataForm.type" />
      </template>
    </div>
  </el-dialog>
</template>
...
import HiDesigner from './design/index.vue'
const propertyJson = { type:0}
export default {
  components: { PrintTemplater,HiDesigner },
  data() {
    return {
      visible: false,
      loading: false,
      ...
      dataForm: {
        ...
        printTemplate: '',
        description: '',
        propertyJson:Object.assign({},propertyJson)
      },
      dataRule: {
        fullName: [
  ...
  methods: {
      ...
      this.$nextTick(() => {
        ...
        if (this.dataForm.id) {
          ...
          getPrintDevInfo(this.dataForm.id).then(res => {
            if (res.data.propertyJson && typeof res.data.propertyJson === 'string') {
              res.data.propertyJson = JSON.parse(res.data.propertyJson)
            }else{
              res.data.propertyJson = Object.assign({},propertyJson)
            }
    ...
    dataFormSubmit() {
      ...
      if (data.propertyJson && typeof data.propertyJson === 'object') {
        ...
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            ...
            getFields(query).then(res => {
              ...
              let sysData =  []
              if (res.data.sysTable) {
                sysData = res.data.sysTable
                delete res.data.sysTable
              }
