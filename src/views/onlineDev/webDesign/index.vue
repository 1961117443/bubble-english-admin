<template>
  <div class="QT-common-layout">
    <div class="QT-common-layout-center">
      ...
      <div class="QT-common-layout-main QT-flex-main">
        <div class="QT-common-head">
          <div  class="qt-opts">
            <el-button v-if="multiTenancy" type="primary" icon="el-icon-plus" @click="AddFromYun()">云端导入</el-button>
            <el-button v-else type="primary" @click="openAddBox()" icon="el-icon-plus">新建</el-button>
            <upload-btn url="/api/visualdev/OnlineDev/Model/Actions/ImportData"
              @on-success="initData" buttonText="文件导入" />              
          </div>
              ...
              <tableOpts @edit="addOrUpdateHandle(scope.row.id)" @del="handleDel(scope.row.id)">
                <el-dropdown>
                  ...
                  <el-dropdown-menu slot="dropdown">
                    ...
                    <el-dropdown-item v-if="multiTenancy" @click.native="ReleaseYun(scope.row.id)">发布云端</el-dropdown-item>
                  </el-dropdown-menu>
                </el-dropdown>
              </tableOpts>
            </template>
    ...
    <el-dialog title="同步菜单" :visible.sync="releaseDialogVisible"
      class="QT-dialog QT-dialog_center release-dialog" lock-scroll width="600px">
      ...
      <div class="dialog-main">
        <div class="item" :class="{'active':releaseQuery.pc===1}" @click="selectToggle('pc')">
          <i class="item-icon icon-qt icon-qt-pc"></i>
        ...
        <div class="item" :class="{'active':releaseQuery.app===1}" @click="selectToggle('app')">
          <i class="item-icon icon-qt icon-qt-mobile"></i>
      ...
      <el-form class="dialog-form-main" :model="releaseQuery" :rules="releaseQueryRule"
        label-position="right" label-width="80px" ref="releaseForm">
        <template v-if="!currRow.pcIsRelease">
          <el-form-item label="上级" prop="pcModuleParentId" v-if="releaseQuery.pc">
            <QT-TreeSelect v-model="releaseQuery.pcModuleParentId" :options="treeData"
              placeholder="选择上级菜单" />
          </el-form-item>
        </template>
        <template v-if="!currRow.appIsRelease">
          ...
          <el-form-item label="上级" prop="appModuleParentId" v-if="releaseQuery.app">
            <QT-TreeSelect v-model="releaseQuery.appModuleParentId" :options="appTreeData"
              placeholder="选择上级菜单" />
          </el-form-item>
        </template>
      </el-form>
    ...
    <importCloud ref="importCloudForm" v-if="importCloudVisible" @close="importCloudVisible=false" />
  </div>
</template>
...
import importCloud from './importCloud.vue'
export default {
  name: 'onlineDev-webDesign',
  mixins: [mixin],
  components: { Form, AddBox, previewDialog,importCloud },
  data() {
    return {
      ...
      treeData: [],
      appTreeData: [],
      importCloudVisible:false
    }
  },
  computed:{
    ...mapGetters(['multiTenancy']),
  },
  methods: {
    preview(id) {
      this.currId = id
    ...
    ReleaseYun(id){
      ...
      this.$request({
        url: `/api/visualdev/base/${id}/Actions/Public`,
        method: 'post',
      }).then(res => {
    ...
    // 云端导入
    AddFromYun(){
      this.importCloudVisible = true
      this.$nextTick(() => {
        this.$refs.importCloudForm.init()
      })
    }
  }
}
